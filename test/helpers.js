    ...
    trx
      ...
      .then(() =>
        Promise.all([
          ...
          trx.raw(
            `ALTER SEQUENCE wimm_spending_list_id_seq minvalue 0 START WITH 1`
          ),
          trx.raw(
            `ALTER SEQUENCE wimm_spending_item_id_seq minvalue 0 START WITH 1`
          ),
...
function seedUsers(db, users) {
  ...
  return db
    ...
    .then(() =>
      ...
      db.raw(`SELECT setval('wimm_user_id_seq', ?)`, [
...
function seedIncomesTables(db, users, incomes) {
  ...
  return db.transaction(async (trx) => {
    ...
    await Promise.all([
      trx.raw(`SELECT setval('wimm_income_id_seq', ?)`, [
...
function seedSpendingListTables(db, users, spendingLists) {
  return db.transaction(async (trx) => {
    ...
    await Promise.all([
      trx.raw(`SELECT setval('wimm_spending_list_id_seq', ?)`, [
...
function seedSpendingItemTables(db, users, spendingLists, spendingItems) {
  return db.transaction(async (trx) => {
    ...
    await Promise.all([
      ...
      trx.raw(`SELECT setval('wimm_spending_item_id_seq', ?)`, [
