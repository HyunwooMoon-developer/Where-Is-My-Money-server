...
describe(`Income Endpoints`, () => {
  ...
  describe(`GET /api/incomes`, () => {
    context("Given no incomes", () => {
      it(`Responds with 200 and an empty list`, () => {
        return supertest(app).get("/api/incomes").expect(200, []);
      });
    });
    context("Given there are incomes in the database", () => {
      beforeEach("Insert incomes", () => {
        return helpers.seedIncomesTables(db, testUsers, testIncome);
      });
      it(`GET /api/incomes responds with 200 and all of the incomes`, () => {
        return supertest(app).get("/api/incomes").expect(200, testIncome);
      });
  ...
  describe(`POST /api/incomes`, () => {
    beforeEach("insert malicious user", () => helpers.seedUsers(db, testUsers));
  ...
  describe(`DELETE /api/incomes/:income_id`, () => {
    ...
    context(`Given there are incomes in the database`, () => {
      ...
      it(`Responds with 204 and removes the notes`, () => {
        const idToRemove = 2;
        const expectedIncome = testIncome.filter(
          (income) => income.id !== idToRemove
        );
        ...
        return supertest(app)
          ...
          .expect(204)
          .then((res) =>
            supertest(app).get("/api/incomes").expect(expectedIncome)
          );
      });
    });
  });
    ...
    context(`Given there are notes in the database`, () => {
      ...
      it(`Responds with 400 when updating only a subset of fields`, () => {
        ...
        return supertest(app)
          ...
          .expect(400, {
            error: {
              message: `Request body must contain either 'start_time', 'end_time', 'hourly_payment', 'daily_extra'`,
            },
          });
      });
