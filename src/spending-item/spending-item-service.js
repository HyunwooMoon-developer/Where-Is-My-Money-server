...
const SpendingItemService = {
  getAllSpendingItem(knex) {
    return knex.select("*").from("wimm_spending_item");
  },
  ...
  getSpendingItemById(knex, id) {
    return knex.from("wimm_spending_item").select("*").where("id", id).first();
  },
  deleteSpendingItem(knex, id) {
    return knex("wimm_spending_item").where({ id }).delete();
  },
  updateSpendingItem(knex, id, updateItem) {
    return knex("wimm_spending_item").where({ id }).update(updateItem);
  },
};
